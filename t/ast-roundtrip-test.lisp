...
(defun %gen-arith-tree (max-depth)
  "A GEN-ONE-OF/GEN-MAP/GEN-TUPLE-composed generator of AST-INT/AST-BINOP trees
up to MAX-DEPTH deep. Built from plain recursive composition of cl-weave's own
combinators rather than GEN-RECURSIVE, so the shape produced is exactly the
node types this file already has fixed expectations for."
  (if (<= max-depth 0)
      (gen-map (lambda (v) (make-ast-int :value v)) (gen-integer :min -20 :max 20))
      (gen-one-of
       (gen-map (lambda (v) (make-ast-int :value v)) (gen-integer :min -20 :max 20))
       (gen-map (lambda (spec)
                  (make-ast-binop :op (first spec) :lhs (second spec) :rhs (third spec)))
                (gen-tuple (gen-member '(+ - *))
                          (%gen-arith-tree (1- max-depth))
                          (%gen-arith-tree (1- max-depth)))))))

(defun %interpret-arith-tree (node)
  "Directly evaluate an AST-INT/AST-BINOP tree without going through
AST-TO-SEXP, so %GEN-ARITH-TREE's property test has an independent oracle to
compare AST-TO-SEXP's output against."
  (if (ast-int-p node)
      (ast-int-value node)
      (funcall (symbol-function (ast-binop-op node))
               (%interpret-arith-tree (ast-binop-lhs node))
               (%interpret-arith-tree (ast-binop-rhs node)))))

(describe-concurrent "ast -> s-expression roundtrip"

(describe "ast-to-sexp — one case per node kind"
  (define-ast-to-sexp-cases
    ...
    ((make-ast-return-from :name 'b :value (make-ast-int :value 1))
     '(return-from b 1))
    ((make-ast-tagbody :tags (list (list 'start (make-ast-int :value 1)) (list 'end)))
     '(tagbody start 1 end))
...
(describe "ast-to-sexp — lambda / defun lambda-list reconstruction"
  ...
  (it-sequential "labels reconstructs bindings and body"
    (expect (ast-to-sexp (make-ast-labels :bindings (list (list 'f '(x) (make-ast-var :name 'x)))
                                          :body (list (make-ast-int :value 1))))
            :to-equal '(labels ((f (x) x)) 1)))

  (it-sequential "defun with documentation and declarations"
  ...
  (it-sequential "defun without documentation omits the string"
    (expect (ast-to-sexp (make-ast-defun :name 'foo :params '(a)
                                         :body (list (make-ast-var :name 'a))))
            :to-equal '(defun foo (a) a))))

(describe "ast-to-sexp — handler-case clause variable presence"
  (it-sequential "clause with a condition variable"
...
(describe "ast-to-sexp — CLOS nodes"
  ...
  (it-sequential "defmethod with a qualifier and an unspecialized param"
  ...
  (it-sequential "minimal defclass has no trailing options"
  ...
  (it-sequential "defclass with default-initargs, metaclass, and sealed"
...
(describe "ast-to-sexp — property: preserves evaluated meaning"
  (it-property "ast-to-sexp of a generated arithmetic tree evaluates to the same value as interpreting the tree directly"
      ((tree (%gen-arith-tree 3)))
    (expect (eval (ast-to-sexp tree)) :to-equal (%interpret-arith-tree tree)))))
